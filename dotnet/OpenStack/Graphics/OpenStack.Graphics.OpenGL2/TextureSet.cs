...
namespace OpenStack.Graphics.OpenGL2
{
    public class TextureSet : IEquatable<TextureSet>
    {
        ...
        {
            if (Environment != textureSet.Environment) return false;
            if (SurfaceIDs.Count != textureSet.SurfaceIDs.Count) return false;
            for (var i = 0; i < SurfaceIDs.Count; i++) if (SurfaceIDs[i] != textureSet.SurfaceIDs[i]) return false;
            return true;
        }
        ...
        public override int GetHashCode()
        {
            ...
            foreach (var surfaceID in SurfaceIDs) hash = (hash * 397) ^ surfaceID.GetHashCode();
            return hash;
        }
    }
}
