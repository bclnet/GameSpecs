...
namespace OpenStack.Graphics.OpenGL2
{
    public class TextureCache
    {
        ...
        public static void Init(bool dispose = true)
        {
            if (dispose)
            {
                if (Textures != null) foreach (var texture in Textures.Values) texture.Dispose();
                if (Uncached != null) foreach (var texture in Uncached) texture.Dispose();
            }
        ...
        static Texture2D LoadTexture(uint textureID, bool useDummy = false, Surface surface = null, IPaletteChanges paletteChanges = null)
        {
            //Console.WriteLine($"--> TextureCache.LoadTexture({textureID:X8})");

            if (textureID >> 24 == 0x04) return LoadPalette(textureID);
            else if (textureID >> 24 == 0x0F) return LoadPaletteSet(textureID);

            var isClipMap = surface != null && surface.Type.HasFlag(SurfaceType.Base1ClipMap);

            Log($"Loading texture {textureID:X8}");

            var texture = DatManager.PortalDat.ReadFromDat<ACE.DatLoader.FileTypes.Texture>(textureID);
            if (texture.SourceData == null)                texture = DatManager.HighResDat.ReadFromDat<ACE.DatLoader.FileTypes.Texture>(textureID);
         ...
         static Texture2D LoadPalette(uint paletteID)
        {
         ...
         static List<Microsoft.Xna.Framework.Color> Padding = Enumerable.Repeat(Microsoft.Xna.Framework.Color.Black, 384).ToList();

         static Texture2D LoadPaletteSet(uint paletteSetID)
        {
         ...
         static readonly int PaletteWidth = 64;

         static Microsoft.Xna.Framework.Color[] LoadPaletteColors(uint paletteID, out int width, out int height)
        {
         ...
         static Texture2D GetTexture2DFromBitmap(GraphicsDevice device, Bitmap bitmap)
        {
         ...
         static byte[] AddAlpha(byte[] rgb)
        {
         ...
         static void ConvertToABGR(byte[] argb)
        {
         ...
         static byte[] IndexToColor(ACE.DatLoader.FileTypes.Texture texture, bool isClipMap = false, PaletteChanges paletteChanges = null)
        {
         ...
         static List<int> GetColors(ACE.DatLoader.FileTypes.Texture texture)
        {
        ...
        public static Texture2D Get(uint fileID, Dictionary<uint, uint> textureChanges = null, PaletteChanges paletteChanges = null, bool useCache = true)
        {
         ...
         static Texture2D GetTexture(uint textureID, Surface surface = null, PaletteChanges paletteChanges = null, bool useCache = true)
        {
