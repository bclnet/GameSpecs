...
namespace OpenStack.Graphics.OpenGL2
{
    ...
    {
        const int MaxTexturesPerAtlas = 2048;
        ...
        public int GetAtlasIdx(SurfaceTexturePalette surfaceTexturePalette)
        {
            ...
            {
                if (CurrentTextureAtlas == null || CurrentTextureAtlas.Textures.Count >= MaxTexturesPerAtlas) TextureAtlases.Add(CurrentTextureAtlas = new TextureAtlas(TextureFormat, TextureAtlases.Count));
        ...
        {
            foreach (var textureAtlas in TextureAtlases) textureAtlas.OnCompleted();
        }

        public void Dispose()
        {
            foreach (var textureAtlas in TextureAtlases) textureAtlas.Dispose();
        }
    }
}
