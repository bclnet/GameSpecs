...
{
    public class TerrainBatchDraw : IDisposable
    {
        ...
        public List<LandVertex> Vertices { get; set; } = new List<LandVertex>();
        public VertexBuffer VertexBuffer { get; set; }
        public int NumItems { get; set; }

        public bool CanAdd(R_Landblock landblock)
            => Vertices.Count + landblock.Landblock.Polygons.Count * 3 <= MaxVertices;
        ...
        LandVertex StartVertex(TextureMergeInfo surfInfo)
        {
            ...
            for (var i = 0; i < surfInfo.TerrainOverlays.Count; i++)
            {
                ...
                if (i == 0) v.TexCoord1.Z = overlayIdx;
                else if (i == 1) v.TexCoord2.Z = overlayIdx;
                else if (i == 2) v.TexCoord3.Z = overlayIdx;
            }
            ...
            if (surfInfo.RoadOverlay != null)
            {
                ...
                if (surfInfo.RoadAlphaOverlays.Count > 1) v.TexCoord5.Z = overlayIdx;
            }
            ...
            for (var i = 0; i < surfInfo.TerrainAlphaOverlays.Count; i++)
            {
                ...
                if (i == 0) v.TexCoord1.W = alphaIdx;
                else if (i == 1) v.TexCoord2.W = alphaIdx;
                else if (i == 2) v.TexCoord3.W = alphaIdx;
            }
            ...
            for (var i = 0; i < surfInfo.RoadAlphaOverlays.Count; i++)
            {
                ...
                var alphaIdx = AlphaAtlasChain.GetTextureIdx(alphaID);
                if (i == 0) v.TexCoord4.W = alphaIdx;
                else if (i == 1) v.TexCoord5.W = alphaIdx;
            }
