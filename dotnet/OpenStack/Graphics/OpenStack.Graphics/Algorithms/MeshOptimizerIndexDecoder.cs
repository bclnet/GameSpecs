...
namespace OpenStack.Graphics.Algorithms
{
    ...
    {
        const byte IndexHeader = 0xe0;

        static void PushEdgeFifo(ValueTuple<uint, uint>[] fifo, ref int offset, uint a, uint b)
        {
            fifo[offset] = (a, b);
            offset = (offset + 1) & 15;
        }

        static void PushVertexFifo(uint[] fifo, ref int offset, uint v, bool cond = true)
        {
            fifo[offset] = v;
            offset = (offset + (cond ? 1 : 0)) & 15;
        }

        static uint DecodeVByte(BinaryReader data)
        {
        ...
        static uint DecodeIndex(BinaryReader data, uint last)
        {
        ...
        static void WriteTriangle(Span<byte> destination, int offset, int indexSize, uint a, uint b, uint c)
        {
            offset *= indexSize;
            if (indexSize == 2)
            {
        ...
        public static byte[] DecodeIndexBuffer(int indexCount, int indexSize, Span<byte> buffer)
        {
            ...
            if (buffer[0] != IndexHeader) throw new ArgumentException("Incorrect index buffer header.");
            ...
            using (var dataReader = new BinaryReader(stream))
            {
                for (var i = 0; i < indexCount; i += 3)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            var b = (feb == 0) ? next++ : vertexFifo[(vertexFifoOffset - feb) & 15];
                            var c = (fec == 0) ? next++ : vertexFifo[(vertexFifoOffset - fec) & 15];
                            if (fea == 15) last = a = DecodeIndex(dataReader, last);
                            if (feb == 15) last = b = DecodeIndex(dataReader, last);
                            if (fec == 15) last = c = DecodeIndex(dataReader, last);
                            WriteTriangle(destination, i, indexSize, a, b, c);
                            PushVertexFifo(vertexFifo, ref vertexFifoOffset, a);
                ...
                if (stream.Position != stream.Length) throw new InvalidDataException("we didn't read all data bytes and stopped before the boundary between data and codeaux table");
            }
