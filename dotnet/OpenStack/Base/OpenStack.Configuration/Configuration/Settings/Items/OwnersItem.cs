...
namespace OpenStack.Configuration
{
    public sealed class OwnersItem : SettingItem
    {
        ...
        SettingText _content;

        public OwnersItem(string owners) : base()
        {
            if (string.IsNullOrEmpty(owners)) throw new ArgumentException("ArgumentCannotBeNullOrEmpty", nameof(owners));

            _content = new SettingText(owners);
            Content = owners.Split(OwnersListSeparator).Select(o => o.Trim()).ToList();
        }

        internal OwnersItem(XElement element, SettingsFile origin) : base(element, origin)
        {
            ...
            if (descendants == null || descendants.Count() != 1 || descendants.FirstOrDefault(d => d is SettingText) == null)
                throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UserSettingUnableToParseConfigFile", "OwnersMustOnlyHaveContent", origin.ConfigFilePath));
        ...
        public override bool Equals(object other)
            => other is OwnersItem owners ? ReferenceEquals(this, owners) ? true
            : Content.OrderedEquals(owners.Content, o => o, StringComparer.Ordinal, StringComparer.Ordinal)
            : false;
        ...
        internal override void Update(SettingItem other)
        {
            ...
            if (!owners.Content.Any()) throw new InvalidOperationException("OwnersItemMustHaveAtLeastOneOwner");
