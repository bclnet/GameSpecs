...
namespace OpenStack.Configuration
{
    public sealed class AuthorItem : TrustedSignerItem
    {
        ...
        protected override IReadOnlyCollection<string> RequiredAttributes { get; } = new HashSet<string>(new[] { ConfigurationConstants.NameAttribute });

        public AuthorItem(string name, params CertificateItem[] certificates) : base(name, certificates) { }

        internal AuthorItem(XElement element, SettingsFile origin) : base(element, origin) { }
        ...
        public override bool Equals(object other)
            => other is AuthorItem author
                ? ReferenceEquals(this, other) ? true : string.Equals(Name, author.Name, StringComparison.Ordinal)
                : false;
