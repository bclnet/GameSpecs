...
namespace OpenStack.Configuration
{
    ...
    public class PackageSourceMappingSourceItem : SettingItem
    {
        ...
        {
            if (string.IsNullOrEmpty(value)) throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "PropertyCannotBeNullOrEmpty", nameof(Key)));
        ...
        public PackageSourceMappingSourceItem(string name, IEnumerable<PackagePatternItem> packagePatternItems) : base()
        {
            if (string.IsNullOrEmpty(name)) throw new ArgumentException("ArgumentCannotBeNullOrEmpty", nameof(name));
            else if (packagePatternItems == null || !packagePatternItems.Any()) throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "ErrorItemNeedsAtLeastOnePackagePattern", name));

            AddAttribute(ConfigurationConstants.KeyAttribute, name);
            Patterns = new List<PackagePatternItem>();
            foreach (PackagePatternItem patternItem in packagePatternItems) Patterns.Add(patternItem);
        }

        internal PackageSourceMappingSourceItem(XElement element, SettingsFile origin) : base(element, origin)
        {
            ...
            if (parsedPackagePatternItems.Count == 0) throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "ErrorItemNeedsAtLeastOnePackagePatternWithPath", Key, origin.ConfigFilePath));
        ...
        internal override void SetOrigin(SettingsFile origin)
        {
            ...
            foreach (PackagePatternItem packagePatternItem in Patterns) packagePatternItem.SetOrigin(origin);
        }

        internal override void RemoveFromSettings()
        {
            base.RemoveFromSettings();
            foreach (PackagePatternItem @namespace in Patterns) @namespace.RemoveFromSettings();
        }

        public override SettingBase Clone()
        {
            var newItem = new PackageSourceMappingSourceItem(Key, Patterns.Select(c => c.Clone() as PackagePatternItem).ToArray());
        ...
        internal override XNode AsXNode()
        {
            ...
            foreach (PackagePatternItem packagePatternItem in Patterns) element.Add(packagePatternItem.AsXNode());
        ...
        internal override void Update(SettingItem other)
        {
            ...
            if (!packageSourceMappingSourceItem.Patterns.Any()) throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "ErrorItemNeedsAtLeastOnePackagePattern", packageSourceMappingSourceItem.Key));
            ...
            {
                if (otherPatterns.TryGetValue(packagePatternItem, out PackagePatternItem otherChild)) otherPatterns.Remove(packagePatternItem);
                if (otherChild == null) { Patterns.Remove(packagePatternItem); packagePatternItem.RemoveFromSettings(); }
                else if (packagePatternItem is SettingItem item) item.Update(otherChild);
            }
        ...
        public override bool Equals(object other)
            ...
            => other is PackageSourceMappingSourceItem item
                ? ReferenceEquals(this, item) ? true : string.Equals(Key, item.Key, StringComparison.OrdinalIgnoreCase)
                : false;
