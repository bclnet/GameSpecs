...
namespace OpenStack.Configuration
{
    public sealed class UnknownItem : SettingItem, ISettingsGroup
    {
        ...
        Dictionary<SettingBase, SettingBase> _mutableChildren;

        internal UnknownItem(XElement element, SettingsFile origin) : base(element, origin)
        {
        ...
        public UnknownItem(string name, IReadOnlyDictionary<string, string> attributes, IEnumerable<SettingBase> children) : base(attributes)
        {
        ...
        internal void Remove(SettingBase setting)
        {
            ...
            if (_mutableChildren.TryGetValue(setting, out var currentSetting) && _mutableChildren.Remove(currentSetting))
            {
                ...
                if (Parent != null && IsEmpty()) Parent.Remove(this);
            }
        }
        ...
        public override bool Equals(object other)
            => !(other is UnknownItem unknown)
                ? false
                : ReferenceEquals(this, unknown) ? true : string.Equals(ElementName, unknown.ElementName, StringComparison.Ordinal);
        ...
        internal override void Update(SettingItem setting)
        {
            ...
            foreach (var child in Children)
            {
                if (otherChildren.TryGetValue(child, out var otherChild)) otherChildren.Remove(child);

                if (otherChild == null) Remove(child);
                else if (child is SettingItem item) item.Update(otherChild as SettingItem);
            }

            foreach (var newChild in otherChildren) Add(newChild.Value);
        }
        ...
        internal void Merge(UnknownItem item)
        {
            ...
            foreach (var attribute in item.Attributes) AddOrUpdateAttribute(attribute.Key, attribute.Value);
            foreach (var child in item.Children)
                if (_mutableChildren.TryGetValue(child, out var existingChild))
                {
                    if (existingChild is SettingItem childItem) childItem.Update(child as SettingItem);
                }
                else _mutableChildren.Add(child, child);
        }

        internal override void SetOrigin(SettingsFile origin)
        {
            base.SetOrigin(origin);
            foreach (var child in _mutableChildren) child.Value.SetOrigin(origin);
        }

        internal override void RemoveFromSettings()
        {
            base.RemoveFromSettings();
            foreach (var child in _mutableChildren) child.Value.RemoveFromSettings();
        }
    }
}
