...
namespace OpenStack.Configuration
{
    ...
    {
        const X509FindType DefaultFindBy = X509FindType.FindByThumbprint;
        const StoreLocation DefaultStoreLocation = StoreLocation.CurrentUser;
        const StoreName DefaultStoreName = StoreName.My;

        public static string GetString(X509FindType type)
            => type.ToString().Replace("FindBy", string.Empty);
        ...
        public StoreClientCertItem(string packageSource, string findValue, StoreLocation? storeLocation = null, StoreName? storeName = null, X509FindType? findBy = null) : base(packageSource)
        {
            if (!storeLocation.HasValue) storeLocation = DefaultStoreLocation;
            if (!storeName.HasValue) storeName = DefaultStoreName;
            if (!findBy.HasValue) findBy = DefaultFindBy;
            if (string.IsNullOrWhiteSpace(findValue)) throw new ArgumentException("ArgumentCannotBeNullOrEmpty", nameof(findValue));

            Update(findValue, storeLocation, storeName, findBy);
        }

        internal StoreClientCertItem(XElement element, SettingsFile origin) : base(element, origin)
        {
            var storeLocation = element.Attribute(XName.Get(ConfigurationConstants.StoreLocationAttribute))?.Value;
            if (string.IsNullOrWhiteSpace(storeLocation)) storeLocation = DefaultStoreLocation.ToString();
            ...
            if (string.IsNullOrWhiteSpace(storeName)) storeName = DefaultStoreName.ToString();
            ...
            if (string.IsNullOrWhiteSpace(findBy)) findBy = GetString(DefaultFindBy);
            ...
            if (string.IsNullOrWhiteSpace(findValue)) throw new ArgumentException("ArgumentCannotBeNullOrEmpty");
        ...
        public X509FindType FindType
            => Enum.TryParse("FindBy" + Attributes[ConfigurationConstants.FindByAttribute], ignoreCase: true, result: out X509FindType result)
                ? result
                : DefaultFindBy;
        ...
        public StoreLocation StoreLocation
            => Enum.TryParse(Attributes[ConfigurationConstants.StoreLocationAttribute], ignoreCase: true, result: out StoreLocation result)
                ? result
                : DefaultStoreLocation;

        public StoreName StoreName
            => Enum.TryParse(Attributes[ConfigurationConstants.StoreNameAttribute], ignoreCase: true, result: out StoreName result)
                ? result
                : DefaultStoreName;
        ...
        protected override IReadOnlyCollection<string> RequiredAttributes { get; } = new HashSet<string>(new[] { ConfigurationConstants.PackageSourceAttribute, ConfigurationConstants.FindValueAttribute });
        ...
        public override SettingBase Clone()
            => new StoreClientCertItem(PackageSource, FindValue, StoreLocation, StoreName, FindType);
        ...
        public override IEnumerable<X509Certificate> Search()
        {
            ...
            try
            {
                ...
                if (foundCertificates.Count == 0) throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "ErrorStoreCertCertificateNotFound", PackageSource, GetString(StoreLocation), GetString(StoreName), GetString(FindType), FindValue));
                return foundCertificates.OfType<X509Certificate>();
            }
            finally { store.Close(); }
        }

        public void Update(string findValue, StoreLocation? storeLocation = null, StoreName? storeName = null, X509FindType? findBy = null)
        {
            if (storeLocation.HasValue) AddOrUpdateAttribute(ConfigurationConstants.StoreLocationAttribute, storeLocation.Value.ToString());
            if (storeName.HasValue) AddOrUpdateAttribute(ConfigurationConstants.StoreNameAttribute, storeName.Value.ToString());
            if (findBy.HasValue) AddOrUpdateAttribute(ConfigurationConstants.FindByAttribute, GetString(findBy.Value));
            if (!string.IsNullOrWhiteSpace(findValue)) AddOrUpdateAttribute(ConfigurationConstants.FindValueAttribute, findValue);
        }
    }
}
