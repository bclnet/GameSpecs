...
namespace OpenStack.Configuration
{
    public sealed class VirtualSettingSection : SettingSection
    {
        ...
        {
            if (!Equals(other)) throw new ArgumentException("Error_MergeTwoDifferentSections");

            foreach (var item in other.Items.Where(item => item != null))
            {
                if (item is ClearItem)
                {
                    if (CanBeCleared) Children.Clear();
                    Children.Add(item);
                    continue;
                }
                ...
                {
                    if (item is UnknownItem unknown) unknown.Merge(currentItem as UnknownItem);
        ...
        internal override bool Add(SettingItem setting)
        {
            ...
            if (!Children.Contains(setting) && !setting.IsEmpty()) { Children.Add(setting); return true; }
            return false;
        }
        ...
        internal override void Remove(SettingItem setting)
        {
            ...
            if (TryGetChild(setting, out var currentSetting))
            {
                ...
                if (currentSetting.Origin != null && currentSetting.Origin.IsMachineWide) throw new InvalidOperationException("CannotUpdateMachineWide");
                if (currentSetting.Origin != null && currentSetting.Origin.IsReadOnly) throw new InvalidOperationException("CannotUpdateReadOnlyConfig");
                if (Children.Remove(currentSetting))
                    // Remove it from the appropriate config
                    if (currentSetting.Parent != null && currentSetting.Parent != this) currentSetting.Parent.Remove(currentSetting);
                if (currentSetting.MergedWith != null)
                    // Add that back to the set since, we should leave the machine wide setting intact.
                    if (!TryRemoveAllMergedWith(currentSetting, out var undeletedItem)) Children.Add(undeletedItem);
            }
        }

        bool TryRemoveAllMergedWith(SettingItem currentSetting, out SettingItem undeletedItem)
        {
            undeletedItem = null;
            ...
            {
                try { elementToDelete.Parent.Remove(elementToDelete); }
                // This means setting was merged with a machine wide settings.
                catch { undeletedItem = elementToDelete; return false; }
            }
            return true;
        }
