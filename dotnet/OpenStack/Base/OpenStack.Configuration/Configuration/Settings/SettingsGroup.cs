...
namespace OpenStack.Configuration
{
    public abstract class SettingsGroup<T> : SettingElement, ISettingsGroup where T : SettingElement
    {
        ...
        protected SettingsGroup() : base() => Children = new List<T>();
        protected SettingsGroup(IReadOnlyDictionary<string, string> attributes, IEnumerable<T> children) : base(attributes) => Children = children == null ? new List<T>() : (IList<T>)new List<T>(children);

        public override bool IsEmpty() => !Children.Any() || Children.All(c => c.IsEmpty());

        internal SettingsGroup(XElement element, SettingsFile origin) : base(element, origin)
        {
        ...
        internal override void SetOrigin(SettingsFile origin)
        {
            ...
            foreach (var child in Children) child.SetOrigin(origin);
        }

        internal override void RemoveFromSettings()
        {
            base.RemoveFromSettings();
            foreach (var child in Children) child.RemoveFromSettings();
        }

        internal virtual bool Add(T setting)
        {
            if (setting == null) throw new ArgumentNullException(nameof(setting));
            else if (Origin.IsMachineWide) throw new InvalidOperationException("CannotUpdateMachineWide");
            else if (Origin.IsReadOnly) throw new InvalidOperationException("CannotUpdateReadOnlyConfig");
        ...
        protected bool TryGetChild(T expectedChild, out T currentChild)
        {
            ...
            foreach (var child in Children)
                if (child.Equals(expectedChild)) { currentChild = child; return true; }
            return false;
        }
