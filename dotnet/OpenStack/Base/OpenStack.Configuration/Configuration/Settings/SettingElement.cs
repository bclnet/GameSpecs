...
namespace OpenStack.Configuration
{
    public abstract class SettingElement : SettingBase
    {
        ...
        /// Default constructor
        /// </summary>
        protected SettingElement() : base()
            => MutableAttributes = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        protected SettingElement(IReadOnlyDictionary<string, string> attributes) : this()
        {
            if (attributes != null)
                foreach (var attribute in attributes)
                    if (IsAttributeValid(attribute.Key, attribute.Value)) MutableAttributes.Add(attribute.Key, attribute.Value);
                    else throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Error_InvalidAttribute", attribute.Key, attribute.Value));
        }
        ...
        internal SettingElement(XElement element, SettingsFile origin) : base(element, origin)
        {
            ValidateAttributes(element, origin);
            MutableAttributes = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            foreach (var existingAttribute in element.Attributes()) MutableAttributes.Add(existingAttribute.Name.LocalName, existingAttribute.Value);
        }

        internal override XNode AsXNode()
        {
            if (Node != null && Node is XElement) return Node;
        ...
        protected void AddOrUpdateAttribute(string attributeName, string value)
        {
            if (!UpdateAttribute(attributeName, value))
                if (value != null) AddAttribute(attributeName, value);
        }

        internal bool UpdateAttribute(string attributeName, string newValue)
        {
            if (string.IsNullOrEmpty(attributeName)) throw new ArgumentException("Argument_Cannot_Be_Null_Or_Empty", nameof(attributeName));
            if (!IsAttributeValid(attributeName, newValue)) throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Error_InvalidAttribute", attributeName, newValue));

            if (Attributes.ContainsKey(attributeName))
            {
                if (newValue == null) MutableAttributes.Remove(attributeName);
                else MutableAttributes[attributeName] = newValue;
                return true;
            }
            return false;
        }
        ...
        protected void AddAttribute(string attributeName, string value)
        {
            if (string.IsNullOrEmpty(attributeName)) throw new ArgumentException("Argument_Cannot_Be_Null_Or_Empty", nameof(attributeName));
            value ??= string.Empty;
            if (!IsAttributeValid(attributeName, value)) throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Error_InvalidAttribute", attributeName, value));
            if (!Attributes.ContainsKey(attributeName)) MutableAttributes[attributeName] = value;
        }

        bool IsAttributeValid(string attributeName, string value)
        {
            if (AllowedAttributes != null)
                // No attributes are allowed
                if (!AllowedAttributes.Any()) return false;
                else if (!AllowedAttributes.Contains(attributeName)) return false;

            if (RequiredAttributes != null)
                // Don't delete any required attributes
                if (value == null && RequiredAttributes.Contains(attributeName)) return false;

            if (AllowedValues != null)
                if (AllowedValues.TryGetValue(attributeName, out var allowed) && !allowed.Contains(value.Trim())) return false;

            if (DisallowedValues != null)
                if (DisallowedValues.TryGetValue(attributeName, out var disallowed) && disallowed.Contains(value.Trim())) return false;

            return true;
        }

        void ValidateAttributes(XElement element, SettingsFile origin)
        {
            if (AllowedAttributes != null)
            {
                if (!AllowedAttributes.Any() && element.HasAttributes)
                    throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UserSettings_UnableToParseConfigFile",
                        string.Format(CultureInfo.CurrentCulture, "NoAttributesAllowed", element.Name.LocalName, element.Attributes().Count()),
                        origin.ConfigFilePath));

                foreach (var attribute in element.Attributes())
                    if (!AllowedAttributes.Contains(attribute.Name.LocalName))
                        throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UserSettings_UnableToParseConfigFile",
                            string.Format(CultureInfo.CurrentCulture, "AttributeNotAllowed", attribute.Name.LocalName, element.Name.LocalName),
                            origin.ConfigFilePath));
            }
            ...
            if (RequiredAttributes != null)
                foreach (var requireAttribute in RequiredAttributes)
                {
                    ...
                    if (attribute == null)
                        throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UserSettings_UnableToParseConfigFile",
                            string.Format(CultureInfo.CurrentCulture, "MissingRequiredAttribute", requireAttribute, element.Name.LocalName),
                            origin.ConfigFilePath));
                }
            ...
            if (AllowedValues != null)
                foreach (var attributeValues in AllowedValues)
                {
                    ...
                    if (attribute != null && !attributeValues.Value.Contains(attribute.Value.Trim()))
                        throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UserSettings_UnableToParseConfigFile",
                            string.Format(CultureInfo.CurrentCulture, "AttributeValueNotAllowed", attribute.Name.LocalName, attribute.Value.Trim(), element.Name.LocalName),
                            origin.ConfigFilePath));
                }
