    ...
    public class PackageSource : IEquatable<PackageSource>
    {
        ...
        public string Source
        {
            ...
            set
            {
                ...
                if (value.StartsWith("https://", StringComparison.OrdinalIgnoreCase)) { _isHttps = true; _isHttp = true; _isLocal = false; }
                else if (value.StartsWith("http://", StringComparison.OrdinalIgnoreCase)) { _isHttps = false; _isHttp = true; _isLocal = false; }
                else { _isHttps = false; _isHttp = false; Uri? uri = TrySourceAsUri; _isLocal = uri != null && uri.IsFile; }
            }
        }
        ...
        public PackageSource(string source) : this(source, source, isEnabled: true) { }
        public PackageSource(string source, string name) : this(source, name, isEnabled: true) { }
        public PackageSource(string source, string name, bool isEnabled) : this(source, name, isEnabled, isOfficial: false) { }
        ...
        public SourceItem AsSourceItem()
        {
            ...
            if (ProtocolVersion != DefaultProtocolVersion) protocolVersion = $"{ProtocolVersion}";
        ...
        public override bool Equals(object? obj)
            => obj is PackageSource source ? Equals(source) : base.Equals(obj);
        ...
        public PackageSource Clone()
            => new PackageSource(Source, Name, IsEnabled, IsOfficial, IsPersistable)
            {
