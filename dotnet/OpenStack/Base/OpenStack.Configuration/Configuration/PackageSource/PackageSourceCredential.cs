    ...
    public class PackageSourceCredential : IEquatable<PackageSourceCredential>
    {
        ...
        public string Password
        {
            get
            {
                ...
                {
                    try { return EncryptionUtility.DecryptString(PasswordText); }
                    catch (NotSupportedException e) { throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UnsupportedDecryptPassword", Source), e); }
                }
                else return PasswordText;
            }
        }
        ...
        public static PackageSourceCredential FromUserInput(string source, string username, string password, bool storePasswordInClearText, string validAuthenticationTypesText)
        {
            ...
            else if (username == null) throw new ArgumentNullException(nameof(username));
            else if (password == null) throw new ArgumentNullException(nameof(password));
            ...
            catch (NotSupportedException e) { throw new OpenStackConfigurationException(string.Format(CultureInfo.CurrentCulture, "UnsupportedEncryptPassword", source), e); }
        }
        ...
        public ICredentials ToICredentials()
            => new AuthTypeFilteredCredentials(new NetworkCredential(Username, Password), ValidAuthenticationTypes);

        internal PackageSourceCredential Clone()
            => new PackageSourceCredential(Source, Username, PasswordText, IsPasswordClearText, ValidAuthenticationTypesText);
        ...
        /// </returns>
        static IEnumerable<string> ParseAuthTypeFilterString(string str)
        ...
        public CredentialsItem AsCredentialsItem()
            => new CredentialsItem(Source, Username, PasswordText, IsPasswordClearText, ValidAuthenticationTypesText);
        ...
        public override bool Equals(object other)
            => Equals(other as PackageSourceCredential);
