...
namespace OpenStack.Configuration
{
    public class PackageSourceProvider : IPackageSourceProvider
    {
        ...
        readonly IEnumerable<PackageSource> _configurationDefaultSources;

        public PackageSourceProvider(ISettings settings) : this(settings, ConfigurationDefaults.Instance.DefaultPackageSources, enablePackageSourcesChangedEvent: true) { }
        public PackageSourceProvider(ISettings settings, IEnumerable<PackageSource> configurationDefaultSources) : this(settings, configurationDefaultSources, enablePackageSourcesChangedEvent: true) { }

        static IEnumerable<PackageSource> LoadConfigurationDefaultSources(IEnumerable<PackageSource> configurationDefaultSources)
        {
#if !IS_CORECLR
            // Global default NuGet source doesn't make sense on Mono
            if (RuntimeEnvironmentHelper.IsMono) return Enumerable.Empty<PackageSource>();
#endif
            var packageSourceLookup = new Dictionary<string, IndexedPackageSource>(StringComparer.OrdinalIgnoreCase);
            var packageIndex = 0;

            foreach (var packageSource in configurationDefaultSources) packageIndex = AddOrUpdateIndexedSource(packageSourceLookup, packageIndex, packageSource, packageSource.Name);
            return packageSourceLookup.Values
                .OrderBy(source => source.Index)
                .Select(source => source.PackageSource);
        }

        static Dictionary<string, IndexedPackageSource> LoadPackageSourceLookup(bool byName, ISettings settings)
        {
        ...
        static Dictionary<string, IndexedPackageSource> LoadPackageSourceLookupByName(ISettings settings)
            => LoadPackageSourceLookup(byName: true, settings);

        Dictionary<string, IndexedPackageSource> LoadPackageSourceLookupBySource()
            => LoadPackageSourceLookup(byName: false, Settings);
        ...
        public IEnumerable<PackageSource> LoadPackageSources()
           => LoadPackageSources(Settings, _configurationDefaultSources);
        ...
        public static IEnumerable<PackageSource> LoadPackageSources(ISettings settings)
            => LoadPackageSources(settings, ConfigurationDefaults.Instance.DefaultPackageSources);

        static List<PackageSource> LoadPackageSources(ISettings settings, IEnumerable<PackageSource> defaultPackageSources)
        {
            var loadedPackageSources = LoadPackageSourceLookupByName(settings).Values
                .OrderBy(source => source.Index)
                .Select(source => source.PackageSource)
                .ToList();
            if (defaultPackageSources != null && defaultPackageSources.Any()) AddDefaultPackageSources(loadedPackageSources, defaultPackageSources);
            return loadedPackageSources;
        }

        static void AddDefaultPackageSources(List<PackageSource> loadedPackageSources, IEnumerable<PackageSource> defaultPackageSources)
        {
            ...
            foreach (var packageSource in defaultPackageSources)
            {
                ...
                if (!sourceMatching && !feedNameMatching) defaultPackageSourcesToBeAdded.Add(packageSource);
            }

            var defaultSourcesInsertIndex = loadedPackageSources.FindIndex(source => source.IsMachineWide);
            if (defaultSourcesInsertIndex == -1) defaultSourcesInsertIndex = loadedPackageSources.Count;
            loadedPackageSources.InsertRange(defaultSourcesInsertIndex, defaultPackageSourcesToBeAdded);
        }

        static PackageSource ReadPackageSource(SourceItem setting, bool isEnabled, ISettings settings)
        {
            var name = setting.Key;
            ...
            if (credentials != null) packageSource.Credentials = credentials;
            ...
            if (clientCertificateItem != null) packageSource.ClientCertificates = clientCertificateItem.Search().ToList();

            packageSource.ProtocolVersion = ReadProtocolVersion(setting);
            return packageSource;
        }

        static int ReadProtocolVersion(SourceItem setting)
            => int.TryParse(setting.ProtocolVersion, out var protocolVersion) ? protocolVersion : PackageSource.DefaultProtocolVersion;

        static int AddOrUpdateIndexedSource(Dictionary<string, IndexedPackageSource> packageSourceLookup, int packageIndex, PackageSource packageSource, string lookupKey)
        {
            ...
            else if (previouslyAddedSource.PackageSource.ProtocolVersion < packageSource.ProtocolVersion && packageSource.ProtocolVersion <= MaxSupportedProtocolVersion)
                // Pick the package source with the highest supported protocol version
                previouslyAddedSource.PackageSource = packageSource;
            return packageIndex;
        }

        static PackageSourceCredential ReadCredential(string sourceName, ISettings settings)
        {
            var environmentCredentials = ReadCredentialFromEnvironment(sourceName);
            if (environmentCredentials != null) return environmentCredentials;
            ...
            return credentialsItem != null && !credentialsItem.IsEmpty()
        ...
        static PackageSourceCredential ReadCredentialFromEnvironment(string sourceName)
        {
            var rawCredentials = Environment.GetEnvironmentVariable("NuGetPackageSourceCredentials_" + sourceName);
            if (string.IsNullOrEmpty(rawCredentials)) return null;
        ...
        PackageSource GetPackageSource(string key, Dictionary<string, IndexedPackageSource> sourcesLookup)
        {
            if (sourcesLookup.TryGetValue(key, out var indexedPackageSource)) return indexedPackageSource.PackageSource;
            ...
            if (_configurationDefaultSources != null && _configurationDefaultSources.Any())
            {
                ...
                foreach (var packageSource in _configurationDefaultSources)
                {
                    ...
                    if (isSourceMatch || isFeedNameMatch) return packageSource;
                }
            }
            return null;
        }
        ...
        public HashSet<string> GetPackageSourceNamesMatchingNamePrefix(string namePrefix)
        {
            ...
            foreach (PackageSource packageSource in packageSources)
                if (packageSource.Name.StartsWith(namePrefix, StringComparison.OrdinalIgnoreCase)) names.Add(packageSource.Name);
            return names;
        }

        public PackageSource GetPackageSourceBySource(string source)
        {
            if (string.IsNullOrEmpty(source)) throw new ArgumentException("ArgumentCannotBeNullOrEmpty", nameof(source));
        ...
        void RemovePackageSource(string name, bool shouldSkipSave, ref bool isDirty)
        {
            // get list of sources
            var packageSourcesSection = Settings.GetSection(ConfigurationConstants.PackageSources);
        ...
        void RemoveDisabledSource(string name, bool shouldSkipSave, ref bool isDirty)
        {
            ...
            if (disableSourcesToRemove != null)
                foreach (var disabledSource in disableSourcesToRemove) { Settings.Remove(ConfigurationConstants.DisabledPackageSources, disabledSource); isDirty = true; }

            if (!shouldSkipSave && isDirty) { Settings.SaveToDisk(); OnPackageSourcesChanged(); isDirty = false; }
        }
        ...
        void UpdatePackageSource(PackageSource newSource, PackageSource existingSource, AddItem existingDisabledSourceItem, CredentialsItem existingCredentialsItem, bool updateEnabled, bool updateCredentials, bool shouldSkipSave, ref bool isDirty)
        {
            if (string.Equals(newSource.Name, existingSource.Name, StringComparison.OrdinalIgnoreCase))
            {
                if ((!string.Equals(newSource.Source, existingSource.Source, StringComparison.OrdinalIgnoreCase) || newSource.ProtocolVersion != existingSource.ProtocolVersion) && newSource.IsPersistable)
                {
                ...
                if (updateEnabled)
                {
                    ...
                    if (!newSource.IsEnabled && existingDisabledSourceItem == null) AddDisabledSource(newSource.Name, shouldSkipSave: true, isDirty: ref isDirty);
                }
        ...
        void AddPackageSource(PackageSource source, bool shouldSkipSave, ref bool isDirty)
        {
            if (source.IsPersistable) { Settings.AddOrUpdate(ConfigurationConstants.PackageSources, source.AsSourceItem()); isDirty = true; }

            if (source.IsEnabled) RemoveDisabledSource(source.Name, shouldSkipSave: true, isDirty: ref isDirty);
            else AddDisabledSource(source.Name, shouldSkipSave: true, isDirty: ref isDirty);

            if (source.Credentials != null && source.Credentials.IsValid()) { Settings.AddOrUpdate(ConfigurationConstants.CredentialsSectionName, source.Credentials.AsCredentialsItem()); isDirty = true; }
        ...
        Dictionary<string, SourceItem> GetExistingSettingsLookup()
        {
            ...
            if (existingSettings != null)
                ...
                {
                    if (existingSettingsLookup.TryGetValue(setting.Key, out var previouslyAddedSetting) && ReadProtocolVersion(previouslyAddedSetting) < ReadProtocolVersion(setting) && ReadProtocolVersion(setting) <= MaxSupportedProtocolVersion)
        ...
        public string DefaultPushSource
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(source)) source = ConfigurationDefaults.Instance.DefaultPushSource;
                return source;
            }
        }
        ...
        {
            if (string.IsNullOrEmpty(name)) throw new ArgumentException("ArgumentCannotBeNulOrEmpty", nameof(name));
        ...
        public void SaveActivePackageSource(PackageSource source)
        {
            try
            {
                ...
                if (activePackageSourceSection != null)
                    foreach (var activePackageSource in activePackageSourceSection.Items) Settings.Remove(ConfigurationConstants.ActivePackageSourceSectionName, activePackageSource);
                Settings.AddOrUpdate(ConfigurationConstants.ActivePackageSourceSectionName, new AddItem(source.Name, source.Source));
                Settings.SaveToDisk();
            }
            catch (Exception) { } // we want to ignore all errors here.
        }

        class IndexedPackageSource
        {
