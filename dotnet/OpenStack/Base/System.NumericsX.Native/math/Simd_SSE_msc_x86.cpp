...
#include "geometry/DominantTri.h"
...
float SSE_Sin(float a) {
	...
	__asm {
		...
		cmpnltss	xmm2, SIMD_SP_halfPI		// xmm2 = ( fabs( PI - a ) >= MathX::HALF_PI ) ? 0xFFFFFFFF : 0x00000000
		movss		xmm3, SIMD_SP_PI			// xmm3 = PI
		xorps		xmm3, xmm1					// xmm3 = PI ^ signbit( PI - a )
		andps		xmm3, xmm2					// xmm3 = ( fabs( PI - a ) >= MathX::HALF_PI ) ? ( PI ^ signbit( PI - a ) ) : 0.0f
		andps		xmm2, SIMD_SP_signBitMask	// xmm2 = ( fabs( PI - a ) >= MathX::HALF_PI ) ? SIMD_SP_signBitMask : 0.0f
		xorps		xmm0, xmm2
		addps		xmm0, xmm3
...
void SSE_Sin4(float a[4], float s[4]) {
	__asm {
		...
		xorps		xmm2, xmm1					// xmm2 = fabs( PI - a )
		cmpnltps	xmm2, SIMD_SP_halfPI		// xmm2 = ( fabs( PI - a ) >= MathX::HALF_PI ) ? 0xFFFFFFFF : 0x00000000
...
void VPCALL SIMD_SSE_Dot(float* dst, const Vector3& constant, const DrawVert* src, const int count) {
	...
	assert((int)&((DrawVert*)0)->xyz == DRAWVERT_XYZ_OFFSET);
...
void VPCALL idSIMD_SSE_AddAssign16(float* dst, const float* src, const int count) {
	__asm {
		mov		ecx, dst
		mov		edx, src
		mov		eax, count
...
// with N in the range [1-6].
// 
// The hot cache clock cycle counts are generally better for the SIMD version than the FPU version. At times up to 40% less clock cycles on a P3. In practise however,
...
void VPCALL SIMD_SSE_MatX_LowerTriangularSolve(const MatrixX& L, float* x, const float* b, const int n, int skip) {
	int nc;
	const float* lptr;
...
// solves x in L'x = b for the n * n sub-matrix of L L has to be a lower triangular matrix with (implicit) ones on the diagonal
// x == b is allowed
void VPCALL SIMD_SSE_MatX_LowerTriangularSolveTranspose(const MatrixX& L, float* x, const float* b, const int n) {
	...
	// if the number of columns is not a multiple of 2 we're screwed for alignment. however, if the number of columns is a multiple of 2 but the number of to be
...
void VPCALL SIMD_SSE_BlendJoints(JointQuat* joints, const JointQuat* blendJoints, const float lerp, const int* index, const int numJoints) {
	...
	for (i = 0; i <= numJoints - 4; i += 4) {
		...
		sinom[0] = MathX::RSqrt(scale0[0]);
		sinom[1] = MathX::RSqrt(scale0[1]);
		sinom[2] = MathX::RSqrt(scale0[2]);
		sinom[3] = MathX::RSqrt(scale0[3]);
	...
	for (; i < numJoints; i++) {
		...
		Vector3& jointVert = joints[n].t;
		const Vector3& blendVert = blendJoints[n].t;
		...
		scale0 = MathX::Sin16((1.0f - lerp) * omega) * sinom;
		scale1 = MathX::Sin16(lerp * omega) * sinom;
...
void VPCALL SIMD_SSE_ConvertJointQuatsToJointMats(JointMat* jointMats, const JointQuat* jointQuats, const int numJoints) {
	...
	assert(sizeof(JointQuat) == JOINTQUAT_SIZE);
	assert(sizeof(JointMat) == JOINTMAT_SIZE);
	assert((int)(&((JointQuat*)0)->t) == (int)(&((JointQuat*)0)->q) + (int)sizeof(((JointQuat*)0)->q));
...
void VPCALL SIMD_SSE_TransformVerts(DrawVert* verts, const int numVerts, const JointMat* joints, const Vector4* weights, const int* index, const int numWeights) {
	...
	assert(sizeof(DrawVert) == DRAWVERT_SIZE);
	assert((int)&((DrawVert*)0)->xyz == DRAWVERT_XYZ_OFFSET);
	assert(sizeof(Vector4) == JOINTWEIGHT_SIZE);
	assert(sizeof(JointMat) == JOINTMAT_SIZE);
	...
	for (j = i = 0; i < numVerts; i++) {
		...
		v = (*(JointMat*)(jointsPtr + index[j * 2 + 0])) * weights[j];
		while (index[j * 2 + 1] == 0) {
			j++;
			v += (*(JointMat*)(jointsPtr + index[j * 2 + 0])) * weights[j];
		}
...
void VPCALL SIMD_SSE_DecalPointCull(byte* cullBits, const Plane* planes, const DrawVert* verts, const int numVerts) {
	...
	for (i = 0; i < numVerts; i += 2) {
		...
		const Vector3& v0 = verts[i + 0].xyz;
		const Vector3& v1 = verts[i + 1].xyz;
	...
	if (numVerts & 1) {
		...
		const Vector3& v = verts[numVerts - 1].xyz;
...
void VPCALL SIMD_SSE_OverlayPointCull(byte* cullBits, Vector2* texCoords, const Plane* planes, const DrawVert* verts, const int numVerts) {
	...
	const Plane& p0 = planes[0];
	const Plane& p1 = planes[1];
	...
	if (numVerts & 1) {
		...
		const Plane& p0 = planes[0];
		const Plane& p1 = planes[1];
		const Vector3& v0 = verts[numVerts - 1].xyz;
...
void VPCALL SIMD_SSE_DeriveTriPlanes(Plane* planes, const DrawVert* verts, const int numVerts, const int* indexes, const int numIndexes) {
	...
	for (i = 0; i <= numIndexes - 12; i += 12) {
		...
		tmp[0] = MathX::RSqrt(tmp[0]);
		tmp[1] = MathX::RSqrt(tmp[1]);
		tmp[2] = MathX::RSqrt(tmp[2]);
		tmp[3] = MathX::RSqrt(tmp[3]);
	...
	for (; i < numIndexes; i += 3) {
		...
		tmp = MathX::RSqrt(n0 * n0 + n1 * n1 + n2 * n2);
...
void VPCALL SIMD_SSE_DeriveTangents(Plane* planes, DrawVert* verts, const int numVerts, const int* indexes, const int numIndexes) {
	...
	assert(sizeof(DrawVert) == DRAWVERT_SIZE);
	assert((int)&((DrawVert*)0)->normal == DRAWVERT_NORMAL_OFFSET);
	assert((int)&((DrawVert*)0)->tangents[0] == DRAWVERT_TANGENT0_OFFSET);
	assert((int)&((DrawVert*)0)->tangents[1] == DRAWVERT_TANGENT1_OFFSET);
	...
	for (; i < numIndexes; i += 3) {
		...
		tmp = MathX::RSqrt(t0 * t0 + t1 * t1 + t2 * t2);
		*(unsigned int*)&tmp ^= signBit[0];
		...
		tmp = MathX::RSqrt(t3 * t3 + t4 * t4 + t5 * t5);
		*(unsigned int*)&tmp ^= signBit[0];
...
void VPCALL SIMD_SSE_DeriveUnsmoothedTangents(DrawVert* verts, const DominantTri* dominantTris, const int numVerts) {
	...
	for (i = 0; i <= numVerts - 4; i += 4) {
		...
		for (j = 0; j < 4; j++) {
			...
			const DominantTri& dt = dominantTris[i + j];
...
void VPCALL SIMD_SSE_NormalizeTangents(DrawVert* verts, const int numVerts) {
	...
	__asm {
		...
		sub			eax, DRAWVERT_SIZE * 4
			...
			// save the 4 DrawVert::normal to project the tangents
			...
			// project and normalize 4 DrawVert::tangent[0]
			...
			// project and normalize 4 DrawVert::tangent[1]
		...
		movss		xmm0, [esi + eax + DRAWVERT_NORMAL_OFFSET + 0]
			...
			// project and normalize one DrawVert::tangent[0]
			...
			// project and normalize one DrawVert::tangent[1]
...
void VPCALL SIMD_SSE_CreateTextureSpaceLightVectors(Vector3* lightVectors, const Vector3& lightOrigin, const DrawVert* verts, const int numVerts, const int* indexes, const int numIndexes) {
	...
	Vector3 localLightOrigin = lightOrigin;
...
void VPCALL SIMD_SSE_CreateSpecularTextureCoords(Vector4* texCoords, const Vector3& lightOrigin, const Vector3& viewOrigin, const DrawVert* verts, const int numVerts, const int* indexes, const int numIndexes) {
	...
	for (int i = 0; i < numVerts; i++) {
		...
		Vector3 lightDir = lightOrigin - v->xyz;
		Vector3 viewDir = viewOrigin - v->xyz;

		float ilength;

		ilength = MathX::RSqrt(lightDir[0] * lightDir[0] + lightDir[1] * lightDir[1] + lightDir[2] * lightDir[2]);
		lightDir[0] *= ilength;
		lightDir[1] *= ilength;
		lightDir[2] *= ilength;

		ilength = MathX::RSqrt(viewDir[0] * viewDir[0] + viewDir[1] * viewDir[1] + viewDir[2] * viewDir[2]);
		viewDir[0] *= ilength;
		viewDir[1] *= ilength;
		viewDir[2] *= ilength;
	...
	for (int i = 0; i < numVerts; i++) {
		...
		temp[0] = MathX::RSqrt(temp[0]);
		temp[1] = MathX::RSqrt(temp[1]);
		temp[2] = MathX::RSqrt(temp[2]);
		temp[3] = MathX::RSqrt(temp[3]);
...
void VPCALL SIMD_SSE_UpSamplePCMTo44kHz(float* dest, const short* src, const int numSamples, const int kHz, const int numChannels) {
	if (kHz == 11025) {
		if (numChannels == 1) {
