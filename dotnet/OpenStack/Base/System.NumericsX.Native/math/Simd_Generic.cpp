...
// dst[i] = constant * src[i];
void VPCALL SIMD_Generic_Mul(float* dst, const float constant, const float* src0, const int count) {
	double c = constant;
...
void VPCALL SIMD_Generic_Div(float* dst, const float constant, const float* divisor, const int count) {
	double c = constant;
...
void VPCALL SIMD_Generic_MinMax(Vector2& min, Vector2& max, const Vector2* src, const int count) {
	min[0] = min[1] = MathX::INFINITY; max[0] = max[1] = -MathX::INFINITY;
#define OPER(X) const Vector2 &v = src[(X)]; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; }
	UNROLL1(OPER)
#undef OPER
}

void VPCALL SIMD_Generic_MinMax(Vector3& min, Vector3& max, const Vector3* src, const int count) {
	min[0] = min[1] = min[2] = MathX::INFINITY; max[0] = max[1] = max[2] = -MathX::INFINITY;
#define OPER(X) const Vector3 &v = src[(X)]; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; } if ( v[2] < min[2] ) { min[2] = v[2]; } if ( v[2] > max[2] ) { max[2] = v[2]; }
	UNROLL1(OPER)
#undef OPER
}
...
void VPCALL SIMD_Generic_MinMax(Vector3& min, Vector3& max, const DrawVert* src, const int count) {
	min[0] = min[1] = min[2] = MathX::INFINITY; max[0] = max[1] = max[2] = -MathX::INFINITY;
#define OPER(X) const Vector3 &v = src[(X)].xyz; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; } if ( v[2] < min[2] ) { min[2] = v[2]; } if ( v[2] > max[2] ) { max[2] = v[2]; }
	UNROLL1(OPER)
#undef OPER
}
...
void VPCALL SIMD_Generic_MinMax(Vector3& min, Vector3& max, const DrawVert* src, const int* indexes, const int count) {
	min[0] = min[1] = min[2] = MathX::INFINITY; max[0] = max[1] = max[2] = -MathX::INFINITY;
#define OPER(X) const Vector3 &v = src[indexes[(X)]].xyz; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; } if ( v[2] < min[2] ) { min[2] = v[2]; } if ( v[2] > max[2] ) { max[2] = v[2]; }
	UNROLL1(OPER)
#undef OPER
}
...
// x == b is allowed
void VPCALL SIMD_Generic_MatX_LowerTriangularSolve(const MatrixX& L, float* x, const float* b, const int n, int skip) {
	int nc;
	const float* lptr;
...
void VPCALL SIMD_Generic_TransformVerts(DrawVert* verts, const int numVerts, const JointMat* joints, const Vector4* weights, const int* index, int numWeights) {
	int i, j;
...
// For each vertex the normalized direction towards the light origin is added to the normalized direction towards the view origin and the result is projected onto texture space.
...
void VPCALL SIMD_Generic_UpSamplePCMTo44kHz(float* dest, const short* src, const int numSamples, const int kHz, const int numChannels) {
	if (kHz == 11025) {
		if (numChannels == 1)
