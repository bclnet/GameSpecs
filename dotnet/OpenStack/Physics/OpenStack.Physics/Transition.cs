...
namespace OpenStack.Physics.Animation
{
    ...
    public class Transition
    {
        ...
        public Vector3 AdjustOffset(Vector3 _offset)
        {
            ...
            {
                if (slidingAngle < 0.0f) checkSlide = true;
                else CollisionInfo.SlidingNormalValid = false;
            }

            if (!CollisionInfo.ContactPlaneValid)
            {
                if (checkSlide) offset -= CollisionInfo.SlidingNormal * slidingAngle;
                return offset;
            }
            ...
            if (checkSlide) offset = Vec.NormalizeCheckSmall(ref slideOffset) ? Vector3.Zero : Vector3.Dot(slideOffset, offset) * slideOffset;
            else if (collisionAngle <= 0.0f) offset -= CollisionInfo.ContactPlane.Normal * collisionAngle;
            else CollisionInfo.ContactPlane.SnapToPlane(ref offset);

            if (CollisionInfo.ContactPlaneCellID == 0 || CollisionInfo.ContactPlaneIsWater) return offset;
            ...
            if (dist >= globSphere.Radius - PhysicsGlobals.EPSILON) return offset;

            var zDist = (globSphere.Radius - dist) / CollisionInfo.ContactPlane.Normal.Z;
            if (globSphere.Radius > Math.Abs(zDist)) SpherePath.AddOffsetToCheckPos(new Vector3(0, 0, zDist));
            return offset;
        }
        ...
        {
            if (SpherePath.BeginPos == null) { offset = Vector3.Zero; offsetPerStep = Vector3.Zero; numSteps = 1; return; }
            ...
            {
                if (step > 1.0f) { numSteps = (int)Math.Ceiling(step); offsetPerStep = offset * (1.0f / numSteps); }
                else if (!offset.Equals(Vector3.Zero)) { offsetPerStep = offset; numSteps = 1; }
                else { offsetPerStep = Vector3.Zero; numSteps = 0; }
                return;
            }

            if (dist <= PhysicsGlobals.EPSILON) { offsetPerStep = Vector3.Zero; numSteps = 0; }
            else { offsetPerStep = offset * (1.0f / step); numSteps = (int)Math.Floor(step) + 1; }
        }
        ...
        public TransitionState CheckOtherCells(ObjCell currCell)
        {
            ...
            for (var i = 0; i < CellArray.Cells.Count; i++)
            {
                ...
                switch (collides)
                {
                    case TransitionState.Slid: CollisionInfo.ContactPlaneValid = false; CollisionInfo.ContactPlaneIsWater = false; return collides;
                    case TransitionState.Collided:
                    case TransitionState.Adjusted: return collides;
                }
            }
        ...
        public bool CheckWalkable(float zCheck)
        {
            ...
            var globSphere = SpherePath.GlobalSphere[0];
            if (SpherePath.NumSphere < 2 && stepHeight > globSphere.Radius * 2) stepHeight = globSphere.Radius * 0.5f;

            if (stepHeight > globSphere.Radius * 2) stepHeight *= 0.5f;
        ...
        public TransitionState CliffSlide(Plane contactPlane)
        {
            ...
            if (Vec.NormalizeCheckSmall(ref collideNormal)) return TransitionState.OK;
            ...
            if (angle <= 0.0f) { SpherePath.AddOffsetToCheckPos(collideNormal * angle); CollisionInfo.SetCollisionNormal(collideNormal); }
        ...
        {
            if (!ObjectInfo.State.HasFlag(ObjectInfoState.OnWalkable) || !ObjectInfo.State.HasFlag(ObjectInfoState.EdgeSlide)) { transitionState = SetEdgeSlide(true, true, TransitionState.OK); return true; }
            if (CollisionInfo.ContactPlaneValid && CollisionInfo.ContactPlane.Normal.Z < zval) { transitionState = SetEdgeSlide(true, false, CliffSlide(CollisionInfo.ContactPlane)); return false; }
            if (SpherePath.Walkable != null) { transitionState = SetEdgeSlide(false, false, SpherePath.PrecipiceSlide(this)); return transitionState == TransitionState.Collided; }
            if (CollisionInfo.ContactPlaneValid) { transitionState = SetEdgeSlide(true, true, TransitionState.OK); return true; }

            SpherePath.AddOffsetToCheckPos(SpherePath.GlobalCurrCenter[0].Center - SpherePath.GlobalSphere[0].Center);
        ...
        public bool FindPlacementPos()
        {
            ...
            if (!SpherePath.PlacementAllowsSliding) return false;
            ...
            if (sphereRad < 0.125f) { fakeSphere = true; adjustRad = 2.0f; }
            else if (sphereRad < 0.48f) sphereRad = 0.48f;
            ...
            if (fakeSphere) fNumSteps *= 0.5f;

            if (fNumSteps <= 1.0f) return false;
        ...
        public bool FindPlacementPosition()
        {
            ...
            if (SpherePath.CheckCell != null)
            {
                ...
                if (transitionState == TransitionState.OK) transitionState = CheckOtherCells(SpherePath.CheckCell);
            }
            else transitionState = TransitionState.Collided;

            var result = ValidatePlacement(transitionState, true);
            if (result != TransitionState.OK) return false;

            SpherePath.InsertType = InsertType.Placement;
            if (!FindPlacementPos()) return false;

            if (!ObjectInfo.StepDown) return ValidatePlacement(TransitionState.OK, true) == TransitionState.OK;
            ...
            if (SpherePath.NumSphere < 2 && globSphere.Radius * 2 < stepDownHeight) stepDownHeight = globSphere.Radius * 0.5f;
        ...
        public bool FindTransitionalPosition()
        {
            ...
            if ((ObjectInfo.State & ObjectInfoState.FreeRotate) != 0) SpherePath.CurPos.Frame.set_rotate(SpherePath.EndPos.Frame.Orientation);
            ...
            for (var step = 0; step < numSteps; step++)
            {
                if ((ObjectInfo.State & ObjectInfoState.IsViewer) != 0)
                {
                    ...
                    if (step == lastStep)
                    {
                        ...
                        if (offsetLen > PhysicsGlobals.EPSILON) { redo = lastStep; offsetPerStep = offset * (offsetLen - SpherePath.LocalSphere[0].Radius * lastStep) / offsetLen; }
                    }
                }
                SpherePath.GlobalOffset = AdjustOffset(offsetPerStep);
                if ((ObjectInfo.State & ObjectInfoState.IsViewer) == 0)
                {
                    if (SpherePath.GlobalOffset.LengthSquared() < PhysicsGlobals.EPSILON * PhysicsGlobals.EPSILON) return (step != 0 && transitionState == TransitionState.OK);
                }
        ...
        public bool FindValidPosition()
            => SpherePath.InsertType == InsertType.Transition ? FindTransitionalPosition() : FindPlacementPosition();
        ...
        public void InitSlidingNormal(Vector3 normal)
        {
            ...
            if (Vec.NormalizeCheckSmall(ref CollisionInfo.SlidingNormal)) CollisionInfo.SlidingNormal = Vector3.Zero;
        }
        ...
        {
            if (cell == null) return TransitionState.Collided;
            ...
            for (var i = 0; i < num_insertion_attempts; i++)
            {
                ...
                switch (transitionState)
                {
                    ...
                    case TransitionState.Collided: return transitionState;
                    case TransitionState.Slid: CollisionInfo.ContactPlaneValid = false; CollisionInfo.ContactPlaneIsWater = false; break;
                }
            }
            return transitionState;
        }
        ...
        public TransitionState PlacementInsert()
        {
            ...
            if (transitState == TransitionState.OK) transitState = CheckOtherCells(checkCell);
        ...
        public bool StepUp(Vector3 collisionNormal)
        {
            ...
            if (ObjectInfo.State.HasFlag(ObjectInfoState.OnWalkable)) { zLandingValue = ObjectInfo.GetWalkableZ(); stepDownHeight = ObjectInfo.StepUpHeight; }
            ...
            if (!stepDown) SpherePath.RestoreCheckPos();
        ...
        {
            if (SpherePath.CheckCell == null) return TransitionState.OK;

            if (num_insertion_attempts <= 0) return TransitionState.Invalid;
            ...
            for (var i = 0; i < num_insertion_attempts; i++)
            {
                ...
                switch (transitState)
                {
                    case TransitionState.OK:
                        ...
                        if (transitState != TransitionState.OK) SpherePath.NegPolyHit = false;

                        if (transitState == TransitionState.Collided) return transitState;
                ...
                if (transitState == TransitionState.OK)
                {
                    if (!SpherePath.Collide)
                    {
                        if (SpherePath.NegPolyHit && !SpherePath.StepDown && !SpherePath.StepUp)
                        {
                            ...
                            {
                                if (!StepUp(SpherePath.NegCollisionNormal)) transitState = SpherePath.StepUpSlide(this);
                            }
                            else
                                transitState = SpherePath.GlobalSphere[0].SlideSphere(this, ref SpherePath.NegCollisionNormal, SpherePath.GlobalCurrCenter[0].Center);
                        }
                        else
                        {
                            if (CollisionInfo.ContactPlaneValid || (ObjectInfo.State & ObjectInfoState.Contact) == 0 || SpherePath.StepDown || SpherePath.CheckCell == null || !ObjectInfo.StepDown) return TransitionState.OK;

                            var zVal = PhysicsGlobals.LandingZ;
                            var stepDownHeight = 0.039999999f;  // set global

                            if ((ObjectInfo.State & ObjectInfoState.OnWalkable) != 0) { zVal = ObjectInfo.GetWalkableZ(); stepDownHeight = ObjectInfo.StepDownHeight; }
                            ...
                            {
                                if (radsum < stepDownHeight) stepDownHeight = SpherePath.GlobalSphere[0].Radius * 0.5f;
                            }

                            if (radsum >= stepDownHeight)
                            {
                                if (StepDown(stepDownHeight, zVal)) { SpherePath.Walkable = null; return TransitionState.OK; }
                            }
                            else
                            {
                                // 2 half-steps
                                stepDownHeight *= 0.5f;
                                if (StepDown(stepDownHeight, zVal) || StepDown(stepDownHeight, zVal)) { SpherePath.Walkable = null; return TransitionState.OK; }
                            }

                            if (EdgeSlide(ref transitState, stepDownHeight, zVal)) return transitState;
                        }
                    }
                    else
                    {
                        ...
                        if (CollisionInfo.ContactPlaneValid && CheckWalkable(PhysicsGlobals.LandingZ))
                        {
                            ...
                            if (transitState != TransitionState.OK) { transitState = TransitionState.OK; reset = true; }
                        }
                        else reset = true;
                        ...
                        if (CollisionInfo.LastKnownContactPlaneValid) { CollisionInfo.LastKnownContactPlaneValid = false; ObjectInfo.StopVelocity(); }
                        else CollisionInfo.SetCollisionNormal(SpherePath.StepUpNormal);
        ...
        public TransitionState ValidatePlacement(TransitionState transitionState, bool adjust)
        {
            ...
            switch (transitionState)
            {
                case TransitionState.OK: SpherePath.CurPos = SpherePath.CheckPos; SpherePath.CurCell = SpherePath.CheckCell; SpherePath.CacheGlobalCurrCenter(); break;
                case TransitionState.Adjusted:
                case TransitionState.Slid: if (adjust) return ValidatePlacement(PlacementInsert(), false); break;
            }
            return transitionState;
        }
        ...
        public TransitionState ValidatePlacementTransition(TransitionState transitionState, ref int redo)
        {
            ...
            switch (transitionState)
            {
                ...
                case TransitionState.Collided:
                case TransitionState.Adjusted:
                case TransitionState.Slid: if (SpherePath.PlacementAllowsSliding && ObjectInfo.TargetID == 0) CollisionInfo.Init(); break; // added target id
            }
            return transitionState;
        }
        ...
        public TransitionState ValidateTransition(TransitionState transitionState, ref int redo)
        {
            ...
            if (transitionState != TransitionState.OK || SpherePath.CheckPos.Equals(SpherePath.CurPos))
            {
                ...
                if (transitionState != TransitionState.OK)
                {
                    if (transitionState != TransitionState.Invalid)
                    {
                        ...
                        if (!CollisionInfo.CollisionNormalValid) { contactPlane.Normal = Vector3.UnitZ; CollisionInfo.SetCollisionNormal(contactPlane.Normal); }
                        SpherePath.SetCheckPos(SpherePath.CurPos, SpherePath.CurCell);
                        ObjCell empty = null;
                    ...
                    if (_redo == 0)
                    {
                        if (CollisionInfo.FramesStationaryFall > 0)
                        {
                            if (CollisionInfo.FramesStationaryFall > 1)
                            {
                                ...
                                if (!ObjectInfo.State.HasFlag(ObjectInfoState.Contact)) { CollisionInfo.SetCollisionNormal(contactPlane.Normal); CollisionInfo.CollidedWithEnvironment = true; }
                            }
                            else CollisionInfo.FramesStationaryFall = 2;
                        }
                        else CollisionInfo.FramesStationaryFall = 1;
                    }
                    else CollisionInfo.FramesStationaryFall = 0;
            ...
            if (CollisionInfo.ContactPlaneValid)
            {
                ...
                if (ObjectInfo.IsValidWalkable(CollisionInfo.ContactPlane.Normal)) ObjectInfo.State |= ObjectInfoState.OnWalkable;
                else ObjectInfo.State &= ~ObjectInfoState.OnWalkable;
            }
            else ObjectInfo.State &= ~(ObjectInfoState.Contact | ObjectInfoState.OnWalkable);
