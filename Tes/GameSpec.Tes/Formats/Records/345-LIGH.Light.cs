...
namespace GameSpec.Tes.Formats.Records
{
    public class LIGHRecord : Record, IHaveEDID, IHaveMODL
    {
        ...
        public struct DATAField
        {
            ...
            public DATAField(BinaryReader r, int dataSize, TesFormat format)
            {
        ...
        public override bool CreateField(BinaryReader r, TesFormat format, string type, int dataSize)
        {
            switch (type)
            {
                ...
                case "FNAM": if (format != TesFormat.TES3) FNAM = r.ReadT<FLTVField>(dataSize); else FULL = r.ReadSTRV(dataSize); return true;
                case "DATA":
