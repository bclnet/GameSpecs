...
namespace GameSpec.Tes.Formats.Records
{
    public class SCPTRecord : Record
    {
        ...
        public struct CTDAField
        {
            ...
            public CTDAField(BinaryReader r, int dataSize, TesFormat format)
            {
        ...
        public override bool CreateField(BinaryReader r, TesFormat format, string type, int dataSize)
        {
            switch (type)
            {
                ...
                case "SCVR": if (format != TesFormat.TES3) SLSDs.Last().SCVRField(r, dataSize); else SCHD.SCVRField(r, dataSize); return true;
                case "SCDA":
