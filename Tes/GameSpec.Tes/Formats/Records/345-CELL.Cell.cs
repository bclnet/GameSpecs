        ...
        public override bool CreateField(BinaryReader r, TesFormat format, string type, int dataSize)
        {
            ...
            if (!InFRMR)
                switch (type)
                {
                    ...
                    case "DATA": DATA = r.ReadINTV(format == TesFormat.TES3 ? 4 : dataSize).ToUI16Field(); if (format == TesFormat.TES3) goto case "XCLC"; return true;
                    case "XCLC": XCLC = r.ReadT<XCLCField>(format == TesFormat.TES3 ? 8 : dataSize); return true;
                    case "XCLL":
