...
namespace GameSpec.Tes.Formats.Records
{
    public class REGNRecord : Record, IHaveEDID
    {
        ...
        public struct RDSDField
        {
            ...
            public RDSDField(BinaryReader r, int dataSize, TesFormat format)
            {
        ...
        public struct RDWTField
        {
            ...
            public static byte SizeOf(TesFormat format) => format == TesFormat.TES4 ? (byte)8 : (byte)12;
            public FormId<WTHRRecord> Weather;
            public uint Chance;
            public FormId<GLOBRecord> Global;

            public RDWTField(BinaryReader r, int dataSize, TesFormat format)
            {
                Weather = new FormId<WTHRRecord>(r.ReadUInt32());
                Chance = r.ReadUInt32();
                Global = format == TesFormat.TES5 ? new FormId<GLOBRecord>(r.ReadUInt32()) : new FormId<GLOBRecord>();
            }
        }
